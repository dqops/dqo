...
public class FileSynchronizationChangeDetectionServiceImpl implements FileSynchronizationChangeDetectionService {
     ...
     * @param dqoUserHomeFileSystemFactory Local file system factory, used to read local files to detect changes.
    ...
    public boolean detectUnsynchronizedChangesInFolder(DqoRoot dqoRoot) {
        ...
        FileSystemService localFileSystemService = userHomeFolderFileSystem.getFileSystemService();

        try (AcquiredSharedReadLock acquiredSharedReadLock = this.userHomeLockManager.lockSharedRead(dqoRoot)) {
            FolderMetadata mostCurrentFolderMetadata = localFileSystemService.listFilesInFolder(
    ...
    public void detectUnsynchronizedChangesInBackground() {
        Schedulers.parallel().schedule(() -> {
            ...
            detectAndPublishLocalFolderStatus(DqoRoot.data_rule_results);
