...
public class BelowPercentPopulationStdev30DaysRule1ParametersSpec extends AbstractRuleParametersSpec {
    private static final ChildHierarchyNodeFieldMapImpl<BelowPercentPopulationStdev30DaysRule1ParametersSpec> FIELDS = new ChildHierarchyNodeFieldMapImpl<>(AbstractRuleParametersSpec.FIELDS) {
        {
        }
    };

    @JsonPropertyDescription("Maximum percent (e.q. 3%) that the current sensor readout could be below a moving average within the time window. Set the time window at the threshold level for all severity levels (warning, error, Fatal) at once. The default is a 14 time periods (days, etc.) time window, but at least 30 readout must exist to run the calculation.")
    private Double percentPopulationBelow = 1.0;

    /**
     * Default constructor.
     */
    public BelowPercentPopulationStdev30DaysRule1ParametersSpec() {
    ...
    public String getRuleDefinitionName() {
        return "stdev/below_percent_population_stdev_30_days";
    }
}