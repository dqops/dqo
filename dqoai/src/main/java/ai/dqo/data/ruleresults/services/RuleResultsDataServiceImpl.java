...
import ai.dqo.data.ruleresults.services.models.CheckResultDetailedSingleModel;
...
public class RuleResultsDataServiceImpl implements RuleResultsDataService {
    ...
    public CheckResultsDetailedDataModel[] readCheckStatusesDetailed(AbstractRootChecksContainerSpec rootChecksContainerSpec,
        ...
        for (Row row : workingTable) {
            ...
            Boolean includeInKpi = row.getBoolean(RuleResultsColumnNames.INCLUDE_IN_KPI_COLUMN_NAME);
            Boolean includeInSla = row.getBoolean(RuleResultsColumnNames.INCLUDE_IN_SLA_COLUMN_NAME);
    ...
    protected Table loadRecentRuleResults(CheckResultsDetailedParameters loadParameters, String connectionName, PhysicalTableName physicalTableName) {
        RuleResultsSnapshot ruleResultsSnapshot = this.ruleResultsSnapshotFactory.createReadOnlySnapshot(connectionName,
                physicalTableName, RuleResultsColumnNames.COLUMN_NAMES_FOR_RESULTS_DETAILED);
        int monthsToLoad = 2;
        ruleResultsSnapshot.ensureNRecentMonthsAreLoaded(loadParameters.getStartMonth(), loadParameters.getEndMonth(), monthsToLoad);
        Table ruleResultsData = ruleResultsSnapshot.getAllData();
        return ruleResultsData;
    }
}
