...
import ai.dqo.checks.column.checkspecs.validity.ColumnValidityNotNullCountRulesSpec;
...
/**
 * Column distinct values count (select count(distinct column_name) from ...) check that counts unique values in a column.
 */
...
public class ColumnUniquenessDistinctCountCheckSpec extends AbstractCheckDeprecatedSpec {
    ...
    @JsonPropertyDescription("Distinct count sensor parameters")
    ...
    @JsonPropertyDescription("Distinct values counts validation rules at various alert severity levels (thresholds)")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
