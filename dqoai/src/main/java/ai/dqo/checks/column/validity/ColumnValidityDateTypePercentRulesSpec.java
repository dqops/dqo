...
/**
 * Column date format percent rules (a list of supported rules).
 */
...
public class ColumnValidityDateTypePercentRulesSpec extends AbstractRuleSetSpec {
    ...
    @JsonPropertyDescription("Minimum date percent at various alert severity levels (thresholds)")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    private MinValueRuleThresholdsSpec minCount;

    @JsonPropertyDescription("Verifies that the date format column percent equals a given number (within a delta)")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
