 ...
 */
public interface FileIndexList extends Iterable<FileIndexWrapper>, HierarchyNode {
    /**
     ...
     */
    FileIndexWrapper getByObjectName(FileIndexName indexName, boolean loadAllWhenMissing);

    /**
     * Creates a new element instance that is marked as new and should be saved on flush.
     * @param indexName Object key (source name).
     * @return Created object instance.
     */
    FileIndexWrapper createAndAddNew(FileIndexName indexName);

    /**
     * Removes a source model. The source is marked for deletion and will be removed on flush.
     * @param indexName Index name to remove.
     * @return True when the model will be removed, false otherwise.
     */
    boolean remove(FileIndexName indexName);

    /**
     * Returns the collection as an immutable list.
     * @return List of file indexes.
     */
    List<FileIndexWrapper> toList();
