...
import ai.dqo.checks.column.checkspecs.numeric.ColumnPercentile25InRangeCheckSpec;
...
@SpringBootTest
public class ColumnNumericPercentile25InRangeSensorParametersSpecBigQueryTests extends BaseTest {
    private ColumnNumericPercentile25InRangeSensorParametersSpec sut;
    ...
    @BeforeEach
    void setUp() {
        this.sut = new ColumnNumericPercentile25InRangeSensorParametersSpec();
    ...
    void getSensorDefinitionName_whenSensorDefinitionRetrieved_thenEqualsExpectedName() {
        Assertions.assertEquals("column/numeric/percentile", this.sut.getSensorDefinitionName());
    }
    ...
    void renderSensor_whenProfilingNoTimeSeriesNoDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
                SELECT MAX(actual_value) AS actual_value, time_period, time_period_utc
                    FROM(
                        SELECT
                            PERCENTILE_CONT((%s), 0.25) OVER (PARTITION BY
                        )
                    AS actual_value         
