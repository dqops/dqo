...
import ai.dqo.sensors.column.numeric.ColumnNumericSampleVarianceInRangeSensorParametersSpec;
...
@SpringBootTest
public class ColumnNumericSampleVarianceInRangeSensorParametersSpecBigQueryTests extends BaseTest {
    private ColumnNumericSampleVarianceInRangeSensorParametersSpec sut;
    ...
    @BeforeEach
    void setUp() {
		this.sut = new ColumnNumericSampleVarianceInRangeSensorParametersSpec();
    ...
    void getSensorDefinitionName_whenSensorDefinitionRetrieved_thenEqualsExpectedName() {
        Assertions.assertEquals("column/numeric/sample_variance", this.sut.getSensorDefinitionName());
    }
    ...
    void renderSensor_whenAdHocOneTimeSeriesNoDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                VAR_SAMP(%s) AS actual_value, analyzed_table.`date` AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
    ...
    void renderSensor_whenCheckpointDefaultTimeSeriesNoDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                VAR_SAMP(%s) AS actual_value, DATE_TRUNC(CAST(CURRENT_TIMESTAMP() AS DATE), MONTH) AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            GROUP BY time_period
            ORDER BY time_period""";
    ...
    void renderSensor_whenAdHocNoTimeSeriesOneDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                VAR_SAMP(%s) AS actual_value, analyzed_table.`length_string` AS stream_level_1
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
    ...
    void renderSensor_whenCheckpointDefaultTimeSeriesOneDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                VAR_SAMP(%s) AS actual_value, analyzed_table.`length_string` AS stream_level_1, DATE_TRUNC(CAST(CURRENT_TIMESTAMP() AS DATE), MONTH) AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
    ...
    void renderSensor_whenPartitionedDefaultTimeSeriesOneDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                VAR_SAMP(%s) AS actual_value, analyzed_table.`length_string` AS stream_level_1, analyzed_table.`date` AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            GROUP BY stream_level_1, time_period
            ORDER BY stream_level_1, time_period""";
    ...
    void renderSensor_whenAdHocOneTimeSeriesThreeDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                VAR_SAMP(%s) AS actual_value, analyzed_table.`strings_with_numbers` AS stream_level_1, analyzed_table.`mix_of_values` AS stream_level_2, analyzed_table.`length_string` AS stream_level_3, analyzed_table.`date` AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
    ...
    void renderSensor_whenCheckpointDefaultTimeSeriesThreeDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                VAR_SAMP(%s) AS actual_value, analyzed_table.`strings_with_numbers` AS stream_level_1, analyzed_table.`mix_of_values` AS stream_level_2, analyzed_table.`length_string` AS stream_level_3, DATE_TRUNC(CAST(CURRENT_TIMESTAMP() AS DATE), MONTH) AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            GROUP BY stream_level_1, stream_level_2, stream_level_3, time_period
            ORDER BY stream_level_1, stream_level_2, stream_level_3, time_period""";
