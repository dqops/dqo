...
public class ColumnUniquenessDistinctCountSensorParametersSpecBigQueryTests extends BaseTest {
    ...
    void renderSensor_whenLevelOneStaticColumnAndNoTimeSeries_thenRendersCorrectSql() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'FR' AS level_1
                        FROM %s AS analyzed_table
                        GROUP BY level_1
                        ORDER BY level_1""",
                JinjaTemplateRenderServiceObjectMother.makeExpectedTableName(runParameters)),
    ...
    void renderSensor_whenLevelOneStaticColumn_thenRendersCorrectSql() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'IT' AS level_1
                        FROM %s AS analyzed_table
                        GROUP BY level_1
                        ORDER BY level_1""",
                JinjaTemplateRenderServiceObjectMother.makeExpectedTableName(runParameters)),
    ...
    void renderSensor_whenLevel1StaticStringAndNoTimeSeries_thenRendersCorrectSql() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'DE' AS level_1
                        FROM %s AS analyzed_table
                        GROUP BY level_1
                        ORDER BY level_1""",
                JinjaTemplateRenderServiceObjectMother.makeExpectedTableName(runParameters)),
    ...
    void renderSensor_whenLevel1StaticStringWithQuoteAndNoTimeSeries_thenRendersCorrectSql() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'DE''s' AS level_1
                        FROM %s AS analyzed_table
                        GROUP BY level_1
                        ORDER BY level_1""",
                JinjaTemplateRenderServiceObjectMother.makeExpectedTableName(runParameters)),
    ...
    void renderSensor_whenLevel1StaticStringLevel2StaticStringAndNoTimeSeries_thenRendersCorrectSql() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'DE' AS level_1, 'PL' AS level_2
                        FROM %s AS analyzed_table
    ...
    void renderSensor_whenLevel1StaticStringLevel2StaticStringLevel3StaticStringAndNoTimeSeries_thenRendersCorrectSql() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'DE' AS level_1, 'PL' AS level_2, 'UK' AS level_3
                        FROM %s AS analyzed_table
    ...
    void renderSensor_whenMissingLevel1Level2StaticStringLevel3StaticStringAndNoTimeSeries_thenRendersCorrectSql() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'PL' AS level_2, 'UK' AS level_3
                        FROM %s AS analyzed_table
    ...
    void renderSensor_whenTimeSeriesDailyAndLevel1StaticAtCheck_thenRendersCorrectSqlWithTimeLevelAsLastGrouping() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, 'US' AS level_1, 'PL' AS level_2, CAST(CURRENT_TIMESTAMP() AS date) AS time_period
                        FROM %s AS analyzed_table
    ...
    void renderSensor_whenLevelOnColumnAndSecondLevelIsStaticValue_thenRendersCorrectSqlWithAliasedColumnReference() {
        ...
        Assertions.assertEquals(String.format("""
                        SELECT
                            count(distinct analyzed_table.`id`) AS actual_value, analyzed_table.`country` AS level_1, 'UK' AS level_2
                        FROM %s AS analyzed_table
                        GROUP BY level_1, level_2
                        ORDER BY level_1, level_2""",
                JinjaTemplateRenderServiceObjectMother.makeExpectedTableName(runParameters)),
