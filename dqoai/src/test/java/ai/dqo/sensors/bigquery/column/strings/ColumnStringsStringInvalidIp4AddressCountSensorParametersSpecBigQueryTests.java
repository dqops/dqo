...
import ai.dqo.checks.column.checkspecs.strings.ColumnStringInvalidIp4AddressCountCheckSpec;
...
import ai.dqo.sensors.column.strings.ColumnStringsStringInvalidIp4AddressCountSensorParametersSpec;
...
@SpringBootTest
public class ColumnStringsStringInvalidIp4AddressCountSensorParametersSpecBigQueryTests extends BaseTest {
    private ColumnStringsStringInvalidIp4AddressCountSensorParametersSpec sut;
    ...
    void setUp() {
        ...
        this.checkSpec = new ColumnStringInvalidIp4AddressCountCheckSpec();
        this.checkSpec.setParameters(this.sut);
    }

    private SensorExecutionRunParameters getRunParametersAdHoc() {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForAdHocCheck(this.sampleTableMetadata, "ip4", this.checkSpec);
    }

    private SensorExecutionRunParameters getRunParametersCheckpoint(CheckTimeScale timeScale) {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForCheckpointCheck(this.sampleTableMetadata, "ip4", this.checkSpec, timeScale);
    }
            ...
            SELECT
                SUM(
                    CASE
                        ...
                        ELSE 1
                    END
                ) AS actual_value, analyzed_table.`result` AS stream_level_1
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            ...
            SELECT
                SUM(
                    CASE
                        ...
                        ELSE 1
                    END
                ) AS actual_value, analyzed_table.`result` AS stream_level_1, DATE_TRUNC(CAST(CURRENT_TIMESTAMP() AS DATE), MONTH) AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            ...
            SELECT
                SUM(
                    CASE
                        ...
                        ELSE 1
                    END
                ) AS actual_value, analyzed_table.`result` AS stream_level_1, analyzed_table.`date` AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            GROUP BY stream_level_1, time_period
            ORDER BY stream_level_1, time_period""";
            ...
            SELECT
                SUM(
                    CASE
                        ...
                        ELSE 1
                    END
                ) AS actual_value, analyzed_table.`result` AS stream_level_1, analyzed_table.`result` AS stream_level_2, analyzed_table.`result` AS stream_level_3, analyzed_table.`date` AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            ...
            SELECT
                SUM(
                    CASE
                        ...
                        ELSE 1
                    END
                ) AS actual_value, analyzed_table.`result` AS stream_level_1, analyzed_table.`result` AS stream_level_2, analyzed_table.`result` AS stream_level_3, DATE_TRUNC(CAST(CURRENT_TIMESTAMP() AS DATE), MONTH) AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            GROUP BY stream_level_1, stream_level_2, stream_level_3, time_period
            ORDER BY stream_level_1, stream_level_2, stream_level_3, time_period""";
