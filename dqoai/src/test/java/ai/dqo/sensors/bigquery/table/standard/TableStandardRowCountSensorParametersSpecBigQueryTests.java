...
public class TableStandardRowCountSensorParametersSpecBigQueryTests extends BaseTest {
    ...
    void renderSensor_whenAdHocNoTimeSeriesOneDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                COUNT(*) AS actual_value, analyzed_table.`strings_with_numbers` AS stream_level_1
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
    ...
    void renderSensor_whenPartitionedDefaultTimeSeriesOneDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                COUNT(*) AS actual_value, analyzed_table.`strings_with_numbers` AS stream_level_1, analyzed_table.`date` AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
            GROUP BY stream_level_1, time_period
            ORDER BY stream_level_1, time_period""";
    ...
    void renderSensor_whenAdHocOneTimeSeriesThreeDataStream_thenRendersCorrectSql() {
        ...
        String target_query = """
            SELECT
                COUNT(*) AS actual_value, analyzed_table.`strings_with_numbers` AS stream_level_1, analyzed_table.`mix_of_values` AS stream_level_2, analyzed_table.`length_string` AS stream_level_3, analyzed_table.`date` AS time_period
            FROM `%s`.`%s`.`%s` AS analyzed_table
            WHERE %s
