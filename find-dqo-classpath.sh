...
if [ -e $(dirname $0)/dqops/pom.xml ]; then
    ...
    if [ -n "$DQO_PREPEND_CLASSES" ]; then
        export DQO_LAUNCH_CLASSPATH="$DQO_PREPEND_CLASSES:$DQO_USER_HOME%/jars/*:$DQO_USER_HOME%/jars/:$(dirname $0)/dqops/target/dqo-dqops-$DQO_VERSION.jar:$(dirname $0)/lib/target/output/dqo-lib-$DQO_VERSION/jars/*"
    else
        export DQO_LAUNCH_CLASSPATH="$DQO_USER_HOME%/jars/*:$DQO_USER_HOME%/jars/:$(dirname $0)/dqops/target/dqo-dqops-$DQO_VERSION.jar:$(dirname $0)/lib/target/output/dqo-lib-$DQO_VERSION/jars/*"
    fi
else
    ...
    if [ -n "$DQO_PREPEND_CLASSES" ]; then
        export DQO_LAUNCH_CLASSPATH="$DQO_PREPEND_CLASSES:$DQO_USER_HOME%/jars/*:$DQO_USER_HOME%/jars/:$DQO_HOME/jars/*"
    else
        export DQO_LAUNCH_CLASSPATH="$DQO_USER_HOME%/jars/*:$DQO_USER_HOME%/jars/:$DQO_HOME/jars/*:$DQO_USER_HOME%/jars/*"
    fi
fi
