...
import com.dqops.checks.column.checkspecs.numeric.ColumnValidLongitudePercentCheckSpec;
...
@SpringBootTest
public class ColumnNumericValidLongitudePercentSensorParametersSpecBigQueryTests extends BaseTest {
    ...
    private SensorExecutionRunParameters getRunParametersProfiling() {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForProfilingCheck(this.sampleTableMetadata, "gcs", this.checkSpec);
    }

    private SensorExecutionRunParameters getRunParametersMonitoring(CheckTimeScale timeScale) {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForMonitoringCheck(this.sampleTableMetadata, "gcs", this.checkSpec, timeScale);
    }

    private SensorExecutionRunParameters getRunParametersPartitioned(CheckTimeScale timeScale, String timeSeriesColumn) {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForPartitionedCheck(this.sampleTableMetadata, "gcs", this.checkSpec, timeScale, timeSeriesColumn);
    }
    ...
    void getSensorDefinitionName_whenSensorDefinitionRetrieved_thenEqualsExpectedName() {
        Assertions.assertEquals("column/numeric/valid_longitude_percent", this.sut.getSensorDefinitionName());
    }
        ...
        String target_query = """
            SELECT
                CASE
                    ...
                    ELSE 100.0 * SUM(
                        CASE
                            WHEN %1$s >= -180.0 AND %1$s <= 180.0 THEN 1
                            ELSE 0
                        END
                    ) / COUNT(%1$s)
                END AS actual_value
