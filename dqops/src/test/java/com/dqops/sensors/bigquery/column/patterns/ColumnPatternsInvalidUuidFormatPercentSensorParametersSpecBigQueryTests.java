...
import com.dqops.checks.column.checkspecs.patterns.ColumnInvalidUuidFormatPercentCheckSpec;
...
@SpringBootTest
public class ColumnPatternsInvalidUuidFormatPercentSensorParametersSpecBigQueryTests extends BaseTest {
    private ColumnPatternsInvalidUuidFormatPercentSensorParametersSpec sut;
    private final String sensorRegex = "r\"^[0-9a-fA-F]{8}[\\s-]?[0-9a-fA-F]{4}[\\s-]?[0-9a-fA-F]{4}[\\s-]?[0-9a-fA-F]{4}[\\s-]?[0-9a-fA-F]{12}$\"";
    private UserHomeContext userHomeContext;
    private ColumnInvalidUuidFormatPercentCheckSpec checkSpec;
    private SampleTableMetadata sampleTableMetadata;

    @BeforeEach
    void setUp() {
        this.sut = new ColumnPatternsInvalidUuidFormatPercentSensorParametersSpec();
        ...
        this.checkSpec = new ColumnInvalidUuidFormatPercentCheckSpec();
        this.checkSpec.setParameters(this.sut);
    }

    private SensorExecutionRunParameters getRunParametersProfiling() {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForProfilingCheck(this.sampleTableMetadata, "uuid", this.checkSpec);
    }

    private SensorExecutionRunParameters getRunParametersMonitoring(CheckTimeScale timeScale) {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForMonitoringCheck(this.sampleTableMetadata, "uuid", this.checkSpec, timeScale);
    }

    private SensorExecutionRunParameters getRunParametersPartitioned(CheckTimeScale timeScale, String timeSeriesColumn) {
        return SensorExecutionRunParametersObjectMother.createForTableColumnForPartitionedCheck(this.sampleTableMetadata, "uuid", this.checkSpec, timeScale, timeSeriesColumn);
    }
    ...
    void getSensorDefinitionName_whenSensorDefinitionRetrieved_thenEqualsExpectedName() {
        Assertions.assertEquals("column/patterns/invalid_uuid_format_percent", this.sut.getSensorDefinitionName());
    }
        ...
        String target_query = """
            SELECT
                CASE
                    ...
                    ELSE 100.0 * SUM(
                        CASE
                            WHEN NOT REGEXP_CONTAINS(SAFE_CAST(%1$s AS STRING), %2$s)
                                THEN 1
                            ELSE 0
                        END
                    ) / COUNT(%1$s)
