...
import com.dqops.checks.column.checkspecs.patterns.ColumnValidUuidFormatPercentCheckSpec;
...
import com.dqops.sensors.column.patterns.ColumnPatternsValidUuidFormatPercentSensorParametersSpec;
...
@SpringBootTest
public class ColumnPatternsValidUuidFormatPercentSensorParametersSpecBigQueryTests extends BaseTest {
    private ColumnPatternsValidUuidFormatPercentSensorParametersSpec sut;
    private final String sensorRegex = "r\"^[0-9a-fA-F]{8}[\\s-]?[0-9a-fA-F]{4}[\\s-]?[0-9a-fA-F]{4}[\\s-]?[0-9a-fA-F]{4}[\\s-]?[0-9a-fA-F]{12}$\"";
    private UserHomeContext userHomeContext;
    private ColumnValidUuidFormatPercentCheckSpec checkSpec;
    private SampleTableMetadata sampleTableMetadata;

    @BeforeEach
    void setUp() {
        this.sut = new ColumnPatternsValidUuidFormatPercentSensorParametersSpec();
        ...
        this.checkSpec = new ColumnValidUuidFormatPercentCheckSpec();
        ...
        String target_query = """
            SELECT
                CASE
                    ...
                    ELSE 100.0 * SUM(
                        CASE
                            WHEN REGEXP_CONTAINS(SAFE_CAST(%1$s AS STRING), %2$s)
                                THEN 1
                            ELSE 0
                        END
                    ) / COUNT(%1$s)
