...
public class DetectedDatatypeEqualsRuleParametersSpec extends AbstractRuleParametersSpec implements RuleMiningRule {
    ...
    public void decreaseRuleSensitivity(CheckResultsNormalizedResult checkResultsSingleCheck) {
        Integer rowIndexWithMostRecentResult = checkResultsSingleCheck.getRowIndexWithMostRecentResult();
        if (rowIndexWithMostRecentResult == null) {
            return;
        }

        Double lastKnownValue = checkResultsSingleCheck.getActualValueColumn().get(rowIndexWithMostRecentResult);
        int code = lastKnownValue != null ? lastKnownValue.intValue() : -1;

        DetectedDatatypeCategory mostRecentDataType = DetectedDatatypeCategory.fromCode(code);
        if (mostRecentDataType != null) {
            this.setExpectedDatatype(mostRecentDataType);
        }
    }
    ...
    public AbstractRuleParametersSpec proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        if (detectedDatatypeCategory == null || detectedDatatypeCategory == DetectedDatatypeCategory.mixed) {
            return null; // we are not detecting these types
        }

        return new DetectedDatatypeEqualsRuleParametersSpec(detectedDatatypeCategory);
    }
}
