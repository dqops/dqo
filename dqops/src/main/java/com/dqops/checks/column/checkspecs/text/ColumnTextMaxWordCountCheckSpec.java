...
public class ColumnTextMaxWordCountCheckSpec
        extends AbstractCheckSpec<ColumnTextMaxWordCountSensorParametersSpec, BetweenIntsRuleParametersSpec, BetweenIntsRuleParametersSpec, BetweenIntsRuleParametersSpec> {
    public static final ChildHierarchyNodeFieldMapImpl<ColumnTextMaxWordCountCheckSpec> FIELDS = new ChildHierarchyNodeFieldMapImpl<>(AbstractCheckSpec.FIELDS) {
        {
        }
    };
    ...
    public String getFriendlyName() {
        return "Verify that the maximum word count of the text is in the range";
    }
    ...
    public boolean proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        if (sourceProfilingCheck.getActualValue() == null) {
            ...
            Optional<Integer> maxRowCount = columnDataAssetProfilingResults
                    .getSampleValues()
                    .stream()
            ...
            sourceProfilingCheck.setActualValue(maxRowCount.get().doubleValue()); // just fake number like there were no invalid values, to enable a check, even if it fails, we cannot calculate a correct value from the samples
