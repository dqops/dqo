...
public class ColumnNotNullsCountCheckSpec
    ...
    public boolean proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        if (sourceProfilingCheck.getActualValue() != null && sourceProfilingCheck.getActualValue() == 0.0) {
            return false; // when there are no not-null values, it is better to configure the "detect empty column" check instead
        }
