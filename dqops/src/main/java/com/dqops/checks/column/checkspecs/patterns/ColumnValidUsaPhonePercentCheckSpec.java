...
public class ColumnValidUsaPhonePercentCheckSpec
        extends AbstractCheckSpec<ColumnPatternsValidUsaPhoneFormatPercentSensorParametersSpec, MinPercentRule100WarningParametersSpec, MinPercentRule100ErrorParametersSpec, MinPercentRule95ParametersSpec> {

    public static final ChildHierarchyNodeFieldMapImpl<ColumnValidUsaPhonePercentCheckSpec> FIELDS = new ChildHierarchyNodeFieldMapImpl<>(AbstractCheckSpec.FIELDS) {
        {
        }
    };

    /**
     * Regular expression pattern for the email that is used to detect valid USA phone numbers during rule mining.
     */
    public static final Pattern USA_PHONE_NUMBER_REGEX_PATTERN = Pattern.compile("^((((\\(\\+1\\)|(\\+1)|(\\([0][0][1]\\)|([0][0][1]))|\\(1\\)|(1))[\\s.-]?)?(\\(?\\d{3}\\)?[\\s.-]?)(\\d{3}[\\s.-]?)(\\d{4})))$");
    ...
    public String getFriendlyName() {
        return "Minimum percentage of rows containing USA phone number values";
    }
    ...
    public boolean proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        if (sourceProfilingCheck.getActualValue() == null) {
            ...
            Double percentOfValidValues = columnDataAssetProfilingResults.matchPercentageOfSamples(value -> {
                ...
                Matcher matcher = USA_PHONE_NUMBER_REGEX_PATTERN.matcher(value.toString());
