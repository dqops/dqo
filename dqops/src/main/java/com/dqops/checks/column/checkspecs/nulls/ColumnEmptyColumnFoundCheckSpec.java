...
public class ColumnEmptyColumnFoundCheckSpec
    ...
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    private MinCountRuleConstant1ParametersSpec warning;
    ...
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    private MinCountRuleConstant1ParametersSpec error;

    @JsonPropertyDescription("Alerting threshold that raises a fatal data quality issue which indicates a serious data quality problem")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    private MinCountRuleConstant1ParametersSpec fatal;
     ...
     */
    @Override
    public MinCountRuleConstant1ParametersSpec getWarning() {
        return this.warning;
    }
     ...
     */
    public void setWarning(MinCountRuleConstant1ParametersSpec warning) {
     ...
     */
    @Override
    public MinCountRuleConstant1ParametersSpec getError() {
        return this.error;
    }
     ...
     */
    public void setError(MinCountRuleConstant1ParametersSpec error) {
     ...
     */
    @Override
    public MinCountRuleConstant1ParametersSpec getFatal() {
        return this.fatal;
    }
     ...
     */
    public void setFatal(MinCountRuleConstant1ParametersSpec fatal) {
    ...
    public boolean proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        if (checkType != CheckType.partitioned) {
            ...
            if (actualValue > 0.0) {
                // some values detected, if there are very few values, we will activate this check

                Long rowCount = tableProfilingResults.getRowCount();
                if (rowCount == null || rowCount < checkMiningConfigurationProperties.getMinReasonableNotNullsCount()) {
                    return false; // the table is too small or we don't know the row count
                }

                if (actualValue > rowCount * miningParameters.getFailChecksAtPercentErrorRows() / 100.0) {
                    return false; // too many not-null values, we can assume that this column will not turn empty, and there is no reason to configure it
                }
            }

            switch (miningParameters.getSeverityLevel()) {
                case warning:
                    this.setWarning(new MinCountRuleConstant1ParametersSpec());
                    break;
                case error:
                    this.setError(new MinCountRuleConstant1ParametersSpec());
                    break;
                case fatal:
                    this.setFatal(new MinCountRuleConstant1ParametersSpec());
                    break;
            }
        ...
        // TODO: consider if we want to configure it for partitioned checks, probably only when the daily row count per partition is above a reasonable limit and we can detect days with no values at all
