...
public class ColumnSchemaColumnExistsCheckSpec
    ...
    public boolean proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        String columnName = this.getHierarchyId().getColumnName();
        ColumnSpec columnSpec = tableSpec.getColumns().get(columnName);
        if (!Strings.isNullOrEmpty(columnSpec.getSqlExpression()) && !columnSpec.getSqlExpression().contains("{column}")) {
            return false; // do not enable on calculated columns, unless they reference self
        }
