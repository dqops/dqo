...
public class ColumnSumInRangeCheckSpec
        extends AbstractCheckSpec<ColumnNumericSumSensorParametersSpec, BetweenFloatsRuleParametersSpec, BetweenFloatsRuleParametersSpec, BetweenFloatsRuleParametersSpec> {
    public static final ChildHierarchyNodeFieldMapImpl<ColumnSumInRangeCheckSpec> FIELDS = new ChildHierarchyNodeFieldMapImpl<>(AbstractCheckSpec.FIELDS) {
        {
        }
    };
    ...
    @JsonPropertyDescription("Default alerting threshold for a sum in range in a column that raises a data quality error (alert).")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    ...
    public String getFriendlyName() {
        return "Sum of numeric values is in the range";
    }
    ...
    public boolean proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        return false; // we may consider proposing values for partitioned checks, because we cannot use the values from the profiling checks, because in growing tables, the sum also grows
    }
}
