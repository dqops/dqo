...
public class ColumnComparisonNotNullCountMatchCheckSpec
        extends AbstractCheckSpec<ColumnNullsNotNullsCountSensorParametersSpec, MaxDiffPercentRule0ParametersSpec, MaxDiffPercentRule1ParametersSpec, MaxDiffPercentRule5ParametersSpec>
        implements ComparisonCheckRules {
    public static final ChildHierarchyNodeFieldMapImpl<ColumnComparisonNotNullCountMatchCheckSpec> FIELDS = new ChildHierarchyNodeFieldMapImpl<>(AbstractCheckSpec.FIELDS) {
        {
        }
    };
    ...
    @JsonPropertyDescription("Warning level threshold to raise a data quality incident with a warning severity level when the count of not null values in the column in the parent table " +
            "and the count of not null values in the compared column (in the reference table) do not match. The alert is generated for every compared group of rows (when data grouping is enabled).")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    private MaxDiffPercentRule0ParametersSpec warning;

    @JsonPropertyDescription("Error level threshold to raise a data quality incident with an error severity level when the count of not null values in the column in the parent table " +
            "and the count of not null values in the compared column (in the reference table) do not match. The alert is generated for every compared group of rows (when data grouping is enabled).")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    private MaxDiffPercentRule1ParametersSpec error;

    @JsonPropertyDescription("Fatal level threshold to raise a data quality incident with a fatal severity level when the count of not null values in the column in the parent table " +
            "and the count of not null values in the compared column (in the reference table) do not match. The alert is generated for every compared group of rows (when data grouping is enabled).")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    ...
    public String getFriendlyName() {
        return "Maximum percentage of difference between the counts of not null values in compared columns";
    }
