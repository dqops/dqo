...
public class ColumnTextMatchDateFormatPercentCheckSpec
    ...
    @JsonPropertyDescription("Default alerting threshold for a maximum percentage of rows with matching date format in a column that raises a data quality error (alert).")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    @JsonSerialize(using = IgnoreEmptyYamlSerializer.class)
    ...
    public String getFriendlyName() {
        return "Minimum percentage of rows containing text values that match a date format";
    }
    ...
    public boolean proposeCheckConfiguration(ProfilingCheckResult sourceProfilingCheck,
        ...
        if (sourceProfilingCheck.getActualValue() == null) {
            ...
            if (percentDaySlashMonthSlashYear != null && (100.0 - percentDaySlashMonthSlashYear) <= miningParameters.getFailChecksAtPercentErrorRows()){
                this.parameters.setDateFormat(DatetimeBuiltInDateFormats.DaySlashMonthSlashYear);
            } else {
                Double percentDayDashMonthDashYear = columnDataAssetProfilingResults.measurePercentOfSamplesMatchingDateFormat(
                        new SimpleDateFormat("dd-MM-yyyy", Locale.ENGLISH));
                if (percentDayDashMonthDashYear != null && (100.0 - percentDayDashMonthDashYear) <= miningParameters.getFailChecksAtPercentErrorRows()){
                    this.parameters.setDateFormat(DatetimeBuiltInDateFormats.DayDashMonthDashYear);
                } else {
                    Double percentDayDotMonthDotYear = columnDataAssetProfilingResults.measurePercentOfSamplesMatchingDateFormat(
                            new SimpleDateFormat("dd.MM.yyyy", Locale.ENGLISH));
                    if (percentDayDotMonthDotYear != null && (100.0 - percentDayDotMonthDotYear) <= miningParameters.getFailChecksAtPercentErrorRows()){
                        this.parameters.setDateFormat(DatetimeBuiltInDateFormats.DayDotMonthDotYear);
                    } else {
                        Double percentISO8601 = columnDataAssetProfilingResults.measurePercentOfSamplesMatchingDateFormat(
                                new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH));
                        if (percentISO8601 != null && (100.0 - percentISO8601) <= miningParameters.getFailChecksAtPercentErrorRows()){
                            this.parameters.setDateFormat(DatetimeBuiltInDateFormats.ISO8601);
                        } else {
                            return false;
                        }
                    }
