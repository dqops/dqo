...
/**
 * ClickHouse source connection provider.
 */
@Component("clickhouse-provider")
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class ClickHouseConnectionProvider extends AbstractSqlConnectionProvider {
    private final BeanFactory beanFactory;
    private final ClickHouseProviderDialectSettings dialectSettings;

    /**
     * Injection constructor.
     * @param beanFactory Bean factory used to create the connection.
     * @param dialectSettings ClickHouse dialect settings.
     */
    @Autowired
    public ClickHouseConnectionProvider(BeanFactory beanFactory,
                                        ClickHouseProviderDialectSettings dialectSettings) {
     ...
     */
    @Override
    public ClickHouseSourceConnection createConnection(ConnectionSpec connectionSpec,
                                                       boolean openConnection,
                                                       SecretValueLookupContext secretValueLookupContext) {
        assert connectionSpec != null;
        ClickHouseSourceConnection connection = this.beanFactory.getBean(ClickHouseSourceConnection.class);
    ...
    public void promptForConnectionParameters(
        ...
        ClickHouseParametersSpec clickhouseSpec = connectionSpec.getClickhouse();
        if (clickhouseSpec == null) {
            clickhouseSpec = new ClickHouseParametersSpec();
            connectionSpec.setClickhouse(clickhouseSpec);
        }

        if (Strings.isNullOrEmpty(clickhouseSpec.getHost())) {
            if (isHeadless) {
                throw new CliRequiredParameterMissingException("--clickhouse-host");
            }

            clickhouseSpec.setHost(terminalReader.prompt("ClickHouse host name (--clickhouse-host)", "${CLICKHOUSE_HOST}", false));
        }

        if (Strings.isNullOrEmpty(clickhouseSpec.getPort())) {
            if (isHeadless) {
                throw new CliRequiredParameterMissingException("--clickhouse-port");
            }

            clickhouseSpec.setPort(terminalReader.prompt("ClickHouse port number (--clickhouse-port)", "${CLICKHOUSE_PORT}", false));
        }

        if (Strings.isNullOrEmpty(clickhouseSpec.getDatabase())) {
            if (isHeadless) {
                throw new CliRequiredParameterMissingException("--clickhouse-database-name");
            }

            clickhouseSpec.setDatabase(terminalReader.prompt("ClickHouse database name (--clickhouse-database-name)", "${CLICKHOUSE_DATABASE_NAME}", false));
        }

        if (Strings.isNullOrEmpty(clickhouseSpec.getUser())) {
            if (isHeadless) {
                throw new CliRequiredParameterMissingException("--clickhouse-user");
            }

            clickhouseSpec.setUser(terminalReader.prompt("ClickHouse user name (--clickhouse-user)", "${CLICKHOUSE_USER}", false));
        }

        if (Strings.isNullOrEmpty(clickhouseSpec.getPassword())) {
            if (isHeadless) {
                throw new CliRequiredParameterMissingException("--clickhouse-password");
            }

            clickhouseSpec.setPassword(terminalReader.prompt("ClickHouse user password (--clickhouse-password)", "${CLICKHOUSE_PASSWORD}", false));
        }
    }
