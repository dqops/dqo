        ...
        try {
            ...
            Mono<DqoJobQueueIncrementalSnapshotModel> returnEmptyWhenError = incrementalJobChanges.doOnError(
                    error -> Mono.just(new DqoJobQueueIncrementalSnapshotModel(
                            new ArrayList<>(), this.synchronizationStatusTracker.getCurrentSynchronizationStatus(principal.getDomainIdentity().getDataDomain()), sequenceNumber)));
            ...
            return new ResponseEntity<>(Mono.just(new DqoJobQueueIncrementalSnapshotModel(
                    new ArrayList<>(), this.synchronizationStatusTracker.getCurrentSynchronizationStatus(principal.getDomainIdentity().getDataDomain()), sequenceNumber)), HttpStatus.OK);
        }
    }
