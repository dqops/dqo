...
/**
 * Sensor runner that introspects the metadata on a table and checks if the column exists. Returns 1.0 when the column was found or 0.0 when the column was not found.
 */
@Component
@Slf4j
@Scope(ConfigurableBeanFactory.SCOPE_SINGLETON)
public class ColumnColumnExistsSensorRunner extends AbstractSensorRunner {
    /**
    ...
    public SensorExecutionResult extractSensorResults(ExecutionContext executionContext,
        ...
        try {
            ...
            Table table = GenericSensorResultsFactory.createResultTableWithResult(introspectedColumnSpec != null ? 1.0 : 0.0,
                    this.defaultTimeZoneProvider.getDefaultTimeZoneId(), sensorPrepareResult.getSensorRunParameters().getTimePeriodGradient());
            return new SensorExecutionResult(sensorRunParameters, table);
        }
            ...
            if (!dummySensorExecution) {
                ...
                try (SourceConnection sourceConnection = connectionProvider.createConnection(connectionSpec, true, secretValueLookupContext)) {
                    ...
                    if (retrievedTableSpecList.size() == 0) {
                        // table not found
                        Table table = GenericSensorResultsFactory.createResultTableWithResult(0.0,
                                this.defaultTimeZoneProvider.getDefaultTimeZoneId(), sensorPrepareResult.getSensorRunParameters().getTimePeriodGradient());
                        return new SensorExecutionResult(sensorRunParameters, table);
                    }
