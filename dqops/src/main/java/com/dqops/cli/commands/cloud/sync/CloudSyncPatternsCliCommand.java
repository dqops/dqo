...
/**
 * 3st level CLI command "cloud sync patterns" to synchronize the "patterns" folder in the DQOps user home.
 */
@Component
@Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
@CommandLine.Command(name = "patterns", header = "Synchronize local \"patterns\" folder that stores default check patters files with DQOps Cloud",
        description = "Uploads any local changes to the cloud and downloads any changes made to the cloud version of the \"patterns\" folder.")
public class CloudSyncPatternsCliCommand extends BaseCommand implements ICommand {
    ...
    public CloudSyncPatternsCliCommand() {
    }

    @Autowired
    public CloudSyncPatternsCliCommand(CloudSynchronizationCliService cloudSynchronizationCliService,
    ...
    public Integer call() throws Exception {
        try {
            ...
            return this.cloudSynchronizationCliService.synchronizeRoot(
                    DqoRoot.patterns, this.mode, this.direction, false, this.isHeadless(), true, principal);
        }
