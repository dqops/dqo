...
export default function TablePreview({ statistics }: tablePreviewProps) {
  ...
  const dataArray = (statistics?.column_statistics || []).map((x) => {
    const newData = {
      null_percent: Number(
        x.statistics?.find((item) => item.collector === 'nulls_percent')?.result
      ),
      null_count: Number(
        x.statistics?.find((item) => item.collector === 'nulls_count')?.result
      ),
      not_null_percent: Number(
        x.statistics?.find((item) => item.collector === 'not_nulls_percent')
          ?.result
      ),
      not_null_count: Number(
        x.statistics?.find((item) => item.collector === 'not_nulls_count')
          ?.result
      ),
      unique_value: Number(
        x.statistics?.find((item) => item.collector === 'distinct_count')
          ?.result
      ),
      unique_percent: Number(
        x.statistics?.find((item) => item.collector === 'distinct_percent')
          ?.result
      ),
      duplicate_value: Number(
        x.statistics?.find((item) => item.collector === 'duplicate_count')
          ?.result
      ),
      duplicate_percent: Number(
        x.statistics?.find((item) => item.collector === 'duplicate_percent')
          ?.result
      ),
      detectedDatatypeVar: getDetectedDatatype(
        Number(
          x.statistics?.find(
      ...
      minimalValue: x.statistics?.find((item) => item.collector === 'min_value')
        ?.result,
      maximumValue: x.statistics?.find((item) => item.collector === 'max_value')
        ?.result,
                ...
                >
                  {renderValue(
                    y.sampleArray?.at(index)
                      ? String(y.sampleArray?.at(index)).slice(0, 50)
                      : ''
                  )}
                </td>
              ))}
