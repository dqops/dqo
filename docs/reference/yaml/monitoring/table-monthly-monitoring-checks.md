...
|<span class="no-wrap-code ">[`monthly_row_count_change`](../../../checks/table/volume/row-count-change.md)</span>|Verifies that the total row count of the tested table has changed by a fixed rate since the last month.|*[TableRowCountChangeCheckSpec](../../../checks/table/volume/row-count-change.md)*| | | |
