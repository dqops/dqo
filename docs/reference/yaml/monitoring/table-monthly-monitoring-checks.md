...
|[comparisons](\docs\reference\yaml\monitoring\table-monthly-monitoring-checks\#tablecomparisonmonthlymonitoringchecksspecmap)|Dictionary of configuration of checks for table comparisons. The key that identifies each comparison must match the name of a data comparison that is configured on the parent table.|[TableComparisonMonthlyMonitoringChecksSpecMap](\docs\reference\yaml\monitoring\table-monthly-monitoring-checks\#tablecomparisonmonthlymonitoringchecksspecmap)| | | |
