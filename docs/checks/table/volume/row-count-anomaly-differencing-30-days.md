...
```
dqo> check run -c=connection_name -t=schema_name.table_name -col=column_name -ch=profile_row_count_anomaly_differencing_30_days
```
...
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
dqo> check run -c=connection_name -t=schema_name.table_name -col=column_name -ch=daily_row_count_anomaly_differencing_30_days
```
**Check structure (YAML)**
```yaml
  monitoring_checks:
