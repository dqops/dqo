...
```
dqo> check enable -c=connection_name -ch=profile_row_count_anomaly_differencing
```
**Run check (Shell)**  
...
```
dqo> check run -c=connection_name -ch=profile_row_count_anomaly_differencing
```
It is additionally feasible to run this check on a specific table. In order to do this, add the table name to the below
```
dqo> check run -c=connection_name -t=table_name -ch=profile_row_count_anomaly_differencing
```
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
dqo> check run -c=connection_name -t=table_name -col=column_name -ch=profile_row_count_anomaly_differencing
```
