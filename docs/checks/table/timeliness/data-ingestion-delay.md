...
```
dqo> check run -c=connection_name -t=table_name -ch=profile_data_ingestion_delay
```
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
dqo> check run -c=connection_name -t=table_name -col=column_name -ch=profile_data_ingestion_delay
```
