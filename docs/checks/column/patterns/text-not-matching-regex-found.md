...
??? info "Samples of generated SQL queries for each data source type"
    ...
    ??? example "ClickHouse"
        ...
        === "Rendered SQL for ClickHouse"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table."target_column") = 0 THEN 0
                    ELSE SUM(
                        CASE
    ...
    ??? example "QuestDB"
        ...
        === "Sensor template for QuestDB"
            ...
            SELECT
                COALESCE(SUM(
                        CASE
                            WHEN {{ lib.render_target_column('analyzed_table') }} !~ {{ lib.render_regex(parameters.regex) }} = TRUE
                                THEN 1
                            ELSE 0
                        END
                    ), 0.0)
        ...
        === "Rendered SQL for QuestDB"
            ...
            SELECT
                COALESCE(SUM(
                        CASE
                            WHEN analyzed_table."target_column" !~ '^((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[0-9][0-9]|[0-9])[.]){3}(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[0-9][0-9]|[0-9])$' = TRUE
                                THEN 1
                            ELSE 0
                        END
                    ), 0.0)
