...
|----------|----------|----------|-----------|-------------|
|string_null_placeholder_percent|adhoc| |[string_null_placeholder_percent](../../../../reference/sensors/column/strings%20column%20sensors/#string-null-placeholder-percent)|[max_percent](../../../../reference/rules/comparison/#max-percent)|
...
=== "Rendered SQL for Snowflake"
    ...
    SELECT
        CASE
            ...
            ELSE 100.0 * SUM(
                CASE
                    WHEN LOWER(TRY_CAST(analyzed_table."target_column" AS VARCHAR)) IN ('null', 'undefined', 'missing', 'nan', 'none', 'na', 'n/a', 'empty', '#n/d', 'blank', '""', '\'\'', '-', '')
                        THEN 1
                    ELSE 0
                END
            ) / COUNT(*)
        ...
        CASE
            ...
            ELSE 100.0 * SUM(
                CASE
                    WHEN LOWER(
        "target_column"
    ) IN ('null', 'undefined', 'missing', 'nan', 'none', 'na', 'n/a', 'empty', '#n/d', 'blank', '""', '\''\''', '-', '')
                        THEN 1
                    ELSE 0
                END
            ) / COUNT(*)
...
To run this check provide check name in [check run command](../../../../command_line_interface/check/#dqo-check-run)
```
dqo.ai> check run -ch=daily_checkpoint_string_null_placeholder_percent
```
It is also possible to run this check on a specific connection. In order to do this, add the connection name to the below
```
dqo.ai> check run -c=connection_name -ch=daily_checkpoint_string_null_placeholder_percent
```
It is additionally feasible to run this check on a specific table. In order to do this, add the table name to the below
```
dqo.ai> check run -c=connection_name -t=table_name -ch=daily_checkpoint_string_null_placeholder_percent
```
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
dqo.ai> check run -c=connection_name -t=table_name -col=column_name -ch=daily_checkpoint_string_null_placeholder_percent
```
**Check structure (Yaml)**
```yaml
      checkpoints:
...
To run this check provide check name in [check run command](../../../../command_line_interface/check/#dqo-check-run)
```
dqo.ai> check run -ch=monthly_checkpoint_string_null_placeholder_percent
```
It is also possible to run this check on a specific connection. In order to do this, add the connection name to the below
```
dqo.ai> check run -c=connection_name -ch=monthly_checkpoint_string_null_placeholder_percent
```
It is additionally feasible to run this check on a specific table. In order to do this, add the table name to the below
```
dqo.ai> check run -c=connection_name -t=table_name -ch=monthly_checkpoint_string_null_placeholder_percent
```
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
dqo.ai> check run -c=connection_name -t=table_name -col=column_name -ch=monthly_checkpoint_string_null_placeholder_percent
```
**Check structure (Yaml)**
```yaml
      checkpoints:
