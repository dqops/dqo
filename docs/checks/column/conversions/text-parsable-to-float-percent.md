...
??? info "Samples of generated SQL queries for each data source type"
    ...
    [text_parsable_to_float_percent](../../../reference/sensors/column/conversions-column-sensors.md#text-parsable-to-float-percent)
        ...
        === "Rendered SQL for ClickHouse"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table."target_column") = 0 THEN 100.0
                    ELSE 100.0 * COUNT(
                        toFloat64OrNull(analyzed_table."target_column")
                    ) / COUNT(analyzed_table."target_column")
                END AS actual_value
        ...
        === "Rendered SQL for DB2"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table."target_column") = 0 THEN 100.0
                    ELSE 100.0 * SUM(
                        CASE WHEN REGEXP_LIKE(analyzed_table."target_column", '^([0-9]+\.?[0-9]*|\.[0-9]+)$') THEN 1 ELSE 0 END
                    ) / COUNT(analyzed_table."target_column")
                END AS actual_value
            FROM(
                SELECT
        ...
        === "Rendered SQL for DuckDB"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table."target_column") = 0 THEN 100.0
                    ELSE 100.0 * SUM(
                        CASE WHEN REGEXP_MATCHES(analyzed_table."target_column", '^([0-9]+\.?[0-9]*|\.[0-9]+)$') IS TRUE THEN 1 ELSE 0 END
                    ) / COUNT(analyzed_table."target_column")
                END AS actual_value
        ...
        === "Rendered SQL for HANA"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table."target_column") = 0 THEN 100.0
                    ELSE 100.0 * SUM(
                        CASE WHEN analyzed_table."target_column" LIKE_REGEXPR '^([0-9]+\.?[0-9]*|\.[0-9]+)$' THEN 1 ELSE 0 END
                    ) / COUNT(analyzed_table."target_column")
                END AS actual_value
            FROM (
                SELECT
    ...
    ??? example "MariaDB"
        ...
        === "Rendered SQL for MariaDB"
            ...
            SELECT
                ...
                analyzed_table.`target_column` RLIKE '^([0-9]+\.?[0-9]*|\.[0-9]+)$'
        ...
        === "Rendered SQL for MySQL"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table.`target_column`) = 0 THEN 100.0
                    ELSE 100.0 * SUM(
                        REGEXP_LIKE(analyzed_table.`target_column`, '^([0-9]+\.?[0-9]*|\.[0-9]+)$')
                    ) / COUNT(analyzed_table.`target_column`)
                END AS actual_value
            FROM `<target_table>` AS analyzed_table
            ```
        ...
        === "Rendered SQL for PostgreSQL"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table."target_column") = 0 THEN 100.0
                    ELSE 100.0 * SUM(
                        CASE WHEN analyzed_table."target_column" ~ '^([0-9]+\.?[0-9]*|\.[0-9]+)$' THEN 1 ELSE 0 END
                    ) / COUNT(analyzed_table."target_column")
                END AS actual_value
        ...
        === "Rendered SQL for Teradata"
            ...
            SELECT
                CASE
                    WHEN COUNT(analyzed_table."target_column") = 0 THEN 100.0
                    ELSE 100.0 * COUNT(
                        REGEXP_SUBSTR(CAST(analyzed_table."target_column" AS VARCHAR(4096)), '^[+-]?[0-9]*[.,]?[0-9]+$')
                    ) / COUNT(analyzed_table."target_column")
                END AS actual_value
