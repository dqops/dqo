...
```
dqo> check run -c=connection_name -t=schema_name.table_name -ch=daily_partition_sum_anomaly_stationary
```
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
dqo> check run -c=connection_name -t=schema_name.table_name -col=column_name -ch=daily_partition_sum_anomaly_stationary
```
