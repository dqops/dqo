...
```
dqo> check enable -c=connection_name -ch=profile_sum_anomaly_differencing_30_days
```
**Run check (Shell)**  
...
```
dqo> check run -c=connection_name -t=schema_name.table_name -col=column_name -ch=profile_sum_anomaly_differencing_30_days
```
...
To enable this check provide connection name and check name in [check enable command](../../../../command-line-interface/check/#dqo-check-enable)
```
dqo> check enable -c=connection_name -ch=daily_sum_anomaly_differencing_30_days
```
**Run check (Shell)**  
To run this check provide check name in [check run command](../../../../command-line-interface/check/#dqo-check-run)
```
...
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
dqo> check run -c=connection_name -t=schema_name.table_name -col=column_name -ch=daily_sum_anomaly_differencing_30_days
```
**Check structure (YAML)**
```yaml
      monitoring_checks:
