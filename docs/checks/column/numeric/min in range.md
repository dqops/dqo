...
|----------|----------|----------|-----------|-------------|
|min_in_range|adhoc| |[min_value](../../../../reference/sensors/column/range%20column%20sensors/#min-value)|[between_floats](../../../../reference/rules/comparison/#between-floats)|
...
=== "Rendered SQL for PostgreSQL"
    ...
    ```
    SELECT
        MIN("target_column") AS actual_value,
...
## **daily checkpoint min in range**  
  
**Check description**  
Verifies that the minimal value in a column is not outside the set range. Stores the most recent row count for each day when the data quality check was evaluated.  
...
To run this check provide check name in [check run command](../../../../command_line_interface/check/#dqo-check-run)
```
dqo.ai> check run -ch=daily_checkpoint_min_in_range
```
It is also possible to run this check on a specific connection. In order to do this, add the connection name to the below
```
dqo.ai> check run -c=connection_name -ch=daily_checkpoint_min_in_range
```
It is additionally feasible to run this check on a specific table. In order to do this, add the table name to the below
```
dqo.ai> check run -c=connection_name -t=table_name -ch=daily_checkpoint_min_in_range
```
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
...
## **monthly checkpoint min in range**  
  
**Check description**  
Verifies that the minimal value in a column does not exceed the set range. Stores the most recent row count for each month when the data quality check was evaluated.  
...
To run this check provide check name in [check run command](../../../../command_line_interface/check/#dqo-check-run)
```
dqo.ai> check run -ch=monthly_checkpoint_min_in_range
```
It is also possible to run this check on a specific connection. In order to do this, add the connection name to the below
```
dqo.ai> check run -c=connection_name -ch=monthly_checkpoint_min_in_range
```
It is additionally feasible to run this check on a specific table. In order to do this, add the table name to the below
```
dqo.ai> check run -c=connection_name -t=table_name -ch=monthly_checkpoint_min_in_range
```
It is furthermore viable to combine run this check on a specific column. In order to do this, add the column name to the below
```
...
```yaml
      checkpoints:
        monthly:
          numeric:
            monthly_checkpoint_min_in_range:
              error:
                from: 10.0
                to: 20.5
              warning:
...
## **monthly partition min in range**  
