...
??? info "Samples of generated SQL queries for each data source type"
    ...
    ??? example "QuestDB"
        ...
        === "Sensor template for QuestDB"
            ...
            SELECT
                COALESCE(100.0 * COUNT({{ lib.render_target_column('analyzed_table') }}) / COUNT(), 0.0) AS actual_value
                {{- lib.render_data_grouping_projections_reference('analyzed_table') }}
                {{- lib.render_time_dimension_projection_reference('analyzed_table') }}
            FROM(
                SELECT
        ...
        === "Rendered SQL for QuestDB"
            ...
            ```sql
            SELECT
                COALESCE(100.0 * COUNT(analyzed_table."target_column") / COUNT(), 0.0) AS actual_value
            FROM(
                SELECT
                    original_table.*
