...
For example when we have a local file system shown above and you want to load the market_dictionary.csv or the sales folder with all files appearing directly in it (without subfolders of sales), set the path prefix to the /usr/share/clients_data.
The path has to be absolute.
