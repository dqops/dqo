...
### **Review details of the execution error in UI**
...
For example, when we execute numeric check on the string column we get a sensor error as the one shown below.
...
left open the **DQOPs usage**, and then **Execution errors** folder.
...
check type, check category or check name. It also displays daily distribution of errors, time when the error occurred, and detailed error message. 
It also displays daily distribution of errors and full errors details including the error message. 
...
## Running checks from DQOps Shell in the debug mode
When running checks from the DQOPs Shell and en error occurs, you can examine its details by executing check in the debug mode.
...
## Reporting errors directly to console when starting dqops/dqo docker image
