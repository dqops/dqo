...
|[*EffectiveScheduleLevelModel*](\docs\client\models\common.md#effectiveschedulelevelmodel)|Enumeration of possible levels at which a schedule could be configured.|
...
|[*CheckResultStatus*](\docs\client\models\check_results.md#checkresultstatus)|Enumeration of check execution statuses. It is the highest severity or an error if the sensor could not be executed due to a configuration issue.|
...
|[*CheckResultSortOrder*](\docs\client\models\incidents.md#checkresultsortorder)|Enumeration of columns names on a {@link CheckResultEntryModel CheckResultEntryModel} that could be sorted.|
...
|[*FolderSynchronizationStatus*](\docs\client\models\jobs.md#foldersynchronizationstatus)|Enumeration of statuses that identify the synchronization status for each folder that could be synchronized to DQOps Cloud.|
...
|[*TableComparisonConfigurationModel*](\docs\client\models\table_comparisons.md#tablecomparisonconfigurationmodel)|Model that contains the basic information about a table comparison configuration that specifies how the current table could be compared to another table that is a source of truth for comparison.|
