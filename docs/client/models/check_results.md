...
The references of all objects used by [check_results](/docs/client/operations/check_results.md) REST API operations are listed below.
...
|<span class="no-wrap-code">`check_result_entries`</span>|Single check results|*List[[CheckResultEntryModel](/docs/client/models/incidents.md#checkresultentrymodel)]*|
...
|<span class="no-wrap-code">[`highest_historical_severity`](/docs/client/models/check_results.md#ruleseveritylevel)</span>|The highest severity of previous executions of this data quality issue in the analyzed time range. It can be different from the *current_severity* if the data quality issue was solved and the most recently data quality issue did not detect it anymore. For partitioned checks, this field returns the same value as the *current_severity*, because data quality issues in older partitions are still valid.|*[RuleSeverityLevel](/docs/client/models/check_results.md#ruleseveritylevel)*|
|<span class="no-wrap-code">[`check_type`](/docs/client/models/table_comparisons.md#checktype)</span>|The check type: profiling, monitoring, partitioned.|*[CheckType](/docs/client/models/table_comparisons.md#checktype)*|
|<span class="no-wrap-code">[`time_scale`](/docs/client/models/common.md#checktimescale)</span>|The check time scale for *monitoring* and *partitioned* check types. The time scales are *daily* and *monthly*. The profiling checks do not have a time scale.|*[CheckTimeScale](/docs/client/models/common.md#checktimescale)*|
