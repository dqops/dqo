...
| partitioned       | (Only available in DqopsAssertTableStatusOperator operator) Boolean flag to configure reading the most recent status of *partitioned* data quality checks. By default, DQOps reads the check execution statuses of ALL partitioned checks in the filtered time range, for all partitions (time periods), since data quality issues can be present in earlier partitions. The default value is True. | Union[Unset, None, bool]                                                           |
