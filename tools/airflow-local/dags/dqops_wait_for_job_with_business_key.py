...
def generate_business_key(dag_id: str) -> str:

    milliseconds_since_epoch = round(time.time() * 1000)

    generator_max: int = 100_000
    for i in range(generator_max):
        yield dag_id + "_" + str(milliseconds_since_epoch) + "_" + str(i)
        # job_ids.append(dag_id + "_" + milliseconds_since_epoch + "_" + i)
...
with DAG(
    ...
    business_key = next(generate_business_key(dag.dag_id))
